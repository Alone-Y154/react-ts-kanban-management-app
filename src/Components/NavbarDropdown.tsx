...
const NavbarDropdown = () => {
  ...
  return (
    <div className="w-[264px] md:hidden mt-4 rounded-lg bg-grey-400 dark:bg-secondary-500 overflow-hidden absolute">
      <p className="text-grey-700 dark:text-grey-400 text-xs font-bold tracking-[2.4px] px-6 py-4">
        ALL BOARDS ({kanban.boards.length})
      </p>
      {kanban.boards.map((board,index) => {
        return(
      <div key={index} className=" flex flex-col pr-6">
        <div onClick={()=>handleNavbarDropdown(board.name , index)} className={`p-4 hover:rounded-r-full flex cursor-pointer items-center gap-3 ${currentPage === board.name ? "text-grey-400 fill-grey-400 bg-primary-700 rounded-r-full": "text-grey-700 fill-grey-700 hover:text-primary-700 hover:fill-primary-700 hover:bg-primary-700 hover:bg-opacity-10 dark:hover:bg-grey-400"}`} >
          <Board />
      ...
      <div className="m-4 w-[235px] dark:bg-secondary-600 rounded-md h-12 bg-grey-500 flex items-center justify-center gap-4">
