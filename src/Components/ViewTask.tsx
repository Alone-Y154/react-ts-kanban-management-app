  ...
  return (
    <div className="w-full flex justify-center items-center h-screen bg-secondary-700 absolute bg-opacity-50">
      <div ref={ref} className="flex flex-col w-[343px] md:w-[480px] p-6 bg-grey-400 rounded-md">
        ...
        <div className="absolute ml-56 mt-8">
        ...
        {viewTaskDetails?.subtasks.map((subtask) => {
          return (
            <div className="w-[295px] md:w-full flex gap-4 items-center p-3 rounded bg-grey-500 mb-2">
              <input
                type="checkbox"
        ...
        <p className="text-xs font-bold text-grey-700 mt-6">Current Status</p>
        {kanban.boards.map((board, index) => {
          if (board.name === currentPage) {
            return (
              <select value={dropdown} onChange={(e)=>setDropdown(e.target.value)} className="mb-2 mt-3 w-[295px] md:w-full h-10 rounded border border-solid outline-none px-4 py-2 text-[13px] font-medium leading-[23px] text-secondary-700 border-grey-700 border-opacity-25">
        ...
        <button
          ...
          className="rounded-[20px] md:w-full mt-5 bg-primary-700 hover:bg-primary-700 hover:text-grey-400 bg-opacity-10 text-primary-700 w-[295px] h-10 flex justify-center items-center text-[13px] font-bold leading-[23px]"
        >
          Save Changes
        </button>
      </div>
